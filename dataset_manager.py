...
class DatasetManager:
    ...
    def __init__(self, training_set, validation_set, test_set, genres, labels):
        ...
        self.validation_list = list(validation_set)
    ...
    def load_images(self, image_name_list, batch_size):
    ...
    def load_labels(self, image_name_list, batch_size):
    ...
    def next_batch(self, batch_size, phase):
        ...
        if phase == 'train':
            ...
            if self.cur_train + batch_size < len(self.training_list):
                ...
                images = self.load_images(images_list, batch_size)
                labels = self.load_labels(images_list, batch_size)
                self.cur_train = new_ptr

        elif phase == 'val':
            images = []
            if self.cur_val + batch_size < len(self.validation_list):
                # print("test normal")
                images = self.load_images(
                    self.validation_list[int(self.cur_val):int(self.cur_val + batch_size)], batch_size)
                labels = self.load_labels(
                    self.validation_list[int(self.cur_val):int(self.cur_val + batch_size)], batch_size
                )
                self.cur_val += batch_size

            else:
                # print("test last")
                new_ptr = (self.cur_val + batch_size) % len(self.validation_list)
                images_list = self.validation_list[int(self.cur_val):] + \
                    self.validation_list[:int(new_ptr)]
                images = self.load_images(images_list, batch_size)
                labels = self.load_labels(images_list, batch_size)
                self.cur_val = new_ptr
